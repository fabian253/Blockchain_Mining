...
if __name__ == "__main__":
    ...
    # get poap contract addresses
    poap_contract_address_list = [config.POAP_CONTRACT_ADDRESS]

    # request nft data for every contract_address
    for idx, contract_address in enumerate(poap_contract_address_list):
        try:
            # request nft data
            ...
            print(
                f"Contract {contract_address} done [{idx +1}/{len(poap_contract_address_list)}]")
        except Exception as e:
            print(
                f"Contract {contract_address} error [{idx+1}/{len(poap_contract_address_list)}]")
